...
@Getter
@Setter
//@JsonTypeName("QuestionWithFeedback")
public class QuestionWithFeedback extends BaseQuestion {
    private String feedback;

    //public QuestionWithFeedback(){};

    public QuestionWithFeedback(Question wrappee, String feedback) {
        super(wrappee);
    ...
    public List<Boolean> validate(List<String> candidates) {
        Set<String> answers = wrappee.getRoot()
            .getAnswers().stream()
    ...
    public QuestionResponse toResponse() {
        return new QuestionWithFeedbackResponse(
            this.getRoot().getUuid(),
            this.getRoot().getQuestion(),
            this.getRoot().getContexts()
                .stream()
                .map(Context::getName)
                .collect(Collectors.toList()),
            this.getRoot().getAnswers()
                .stream()
                .map(Answer::getAnswer)
                .collect(Collectors.toList()),
            this.getFeedback()
