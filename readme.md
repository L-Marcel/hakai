...
- [Resumo](#resumo)
- [Visão geral desta instância](#visão-geral-desta-instância)
...
Este repositório se trata de uma das instâncias da proposta de `framework` para a criação de aplicações de perguntas e respostas baseadas similares ao `Kahoot`, porém, que utilizam algum tipo de Agente (`IA`) para elaborar variações de questões. O `framework` está disponível em:

- [Kahai (framework)](https://github.com/l-marcel/hakai);

Tudo foi completamente desenvolvido durante a disciplina de `PDS - Projeto de Detalhado de Software` e, para testá-lo, foram desenvolvidas três instâncias (aplicações). Sendo elas:
...
# Visão geral desta instância

- Gerenciamento de salas (parcial):
    - O dono da sala que estabelece quando fechar a sala;
    - Sem limite de tempo para fechar a sala.
- Estratégia de distribuição de variações de questões:
    - As variações são enviadas uma por vez para os participantes;
    - As questões são distribuídas pela dificuldade e última resposta do participante.
- Estratégia de pontuação:
    - Valor atribuído com base na dificuldade da questão;
    - Erro não decresce a pontuação.
- Variedade de tipos de questões:
    - Questões objetivas de única escolha.
...
Vale destacar que você precisará criar um arquivo `backend/src/main/resources/secret.properties`, responsável por armazenar nossas chaves secretas do [Open Router](https://openrouter.ai/) e outras configurações. Um exemplo das configurações disponíveis pode ser visto em:

- [Kahai (propriedades)](https://github.com/L-Marcel/kahai?tab=readme-ov-file#chaves-modelo-da-ia-etc).
